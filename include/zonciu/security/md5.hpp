...
{
public:
    Md5(const char* data, size_t length) { _Make(data, length); }
    Md5(std::string& data) { _Make(data.c_str(), data.length()); }
...
private:
    ...
    void _Make(const char*data, size_t length)
    {
        static const uint32_t _s[64] = {
        ...
        for (size_t i = 0; i < chunk_count; i++)
        {
            ...
            for (uint32_t j = 0; j < 64; j++)
            {
                ...
                d = c;
                c = b;
                b = b + _LeftRotate(a + f + _k[j] + message[(i << 4) + g], _s[j]);
                a = tmpd;
            }
            _raw[0] += a;
            _raw[1] += b;
    ...
    uint32_t _LeftRotate(uint32_t x, uint32_t n)
    {
        return ((x << n) | (x >> (32 - n)));
    }
};
